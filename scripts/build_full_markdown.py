...
md_content = """# CoVA-TAD: In-Context Predictive Dispersion for Training-Free Tabular Anomaly Detection
...
$$v_c(x) = \\frac{1}{999}\\sum_{k=1}^{999}\\bigl(q_{c,k}(x) - \\bar{q}_c(x)\\bigr)^2, \\quad \\bar{q}_c(x) = \\frac{1}{999}\\sum_{k=1}^{999} q_{c,k}(x).$$
...
**Code availability.** An anonymized code repository accompanies this submission at [https://anonymous.4open.science/r/TODO](https://anonymous.4open.science/r/TODO) (and via the supplementary code archive), containing the complete CoVA-TAD estimator, data loader, benchmark runner, Isolation Forest control, and reproducible figure and table generation scripts.
...
Substituting gives $s_*^2(x_*) \to k(x_*, x_*) + \sigma_\epsilon^2 - 0 = \kappa + \sigma_\epsilon^2$. $\blacksquare$
...
The code, configuration files, and benchmark runners are hosted anonymously at [https://anonymous.4open.science/r/TODO](https://anonymous.4open.science/r/TODO). From a clean Python 3.10–3.12 environment, the following commands reproduce the result CSV, statistics, tables, and figures:
...
| **Gaussian NLL** | $\sum_{c} \bigl[\frac{(x_c - \mu_c)^2}{\sigma_c^2 + 10^{-4}} + \log(\sigma_c^2 + 10^{-4})\bigr]$ | 0.8592 | 0.7883 | 0.7432 | 3 / 10 |
...
## Appendix N: Checklist, Limitations, and Intended Use

- **Reference assumption.** Benchmark reference cohorts are guaranteed anomaly-free by construction. Real-world collections may be contaminated; evaluating robustness under noisy reference sets is an important open direction.
- **Repetition and seeds.** Primary results use seed 42. Six-seed stability results are reported in Table 1 of the main paper.
- **Comparator provenance.** Isolation Forest is locally re-run; all other baseline metrics are transcribed from published tables. Direct local comparison with OFA-TAD and MCM is outstanding.
...
- **Compute.** CPU wall-clock times are reported for 14 CoVA-TAD datasets and all 15 Isolation Forest controls. GPU and cross-hardware comparisons are outstanding.
...
with open("paper/main_paper.md", "w") as f:
    f.write(md_content.strip() + "\n")

print("Successfully wrote full unabridged paper/main_paper.md")
